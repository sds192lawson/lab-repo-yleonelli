...
open_payments_dates_cleaned |>
  ...
  map(is.Date)
##TA Feedback: Try using map_lgl here instead of map()! +2.5
...
get_doctor_name <- function(doctor_id){
  open_payments |>
    ...
    summarize(Covered_Recipient_Full_Name = first(Covered_Recipient_Full_Name)) |> ##TA Feedback use filter() here instead of first()
...
##TA Feedback: otherwise good job on this question! +2
##Total for Q1-Q7: 19.5 pts
```
...
##TA Feedback: This look beautiful! +3
##Total for Q1-Q8: 22.5 pts
```
...
payments_calendar <- function(doctor_id, doctor_name) {
  open_payments |>
    filter(Covered_Recipient_NPI == doctor_id) |> ##TA Feedback: You need a summarize function here:  summarize(specialty = first(Covered_Recipient_Specialty_1)) |>
   # pull()
    ggplot(aes(
...
payment_calendar_plots <- map2(
  ...
  payments_calendar)
##TA Feedback: Otherwise nice job here! +2 
