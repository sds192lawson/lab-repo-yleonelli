...
#| eval: true
hmda_ms_2024 |>
  ggplot(aes(x = loan_purpose )) + geom_bar() +
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024",
       x = "Purpose of Loan",
       y = "Count of Loans") +
  theme_minimal()
##TA Feedback: This graph looks great except for one small thing, the y-variable in this case is the count of applications not the count of granted loans. -0.5 for mislabeling
##Total for Q1: 2 pts
```
:::
...
```{r}
race_count<- hmda_ms_2024 |>
  filter(derived_race != "Free Form Text Only")|>
  filter(derived_race != "Race Not Available")
race_count

# This code was done to remove values not needed, such as Race Not Available and Free Form Text Only
...
race_count |>
  ggplot(aes(x = derived_race )) + geom_bar() +
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024: Applicants and their Race",
       x = "Race Category",
       y = "Count of Applicants") +
  coord_flip()
  theme_minimal()
  ##TA Feedback: This graph looks awesome except for the same small labeling mistake as in question 1. -0.5 for minor mislabeling
...
#| eval: true
hmda_ms_2024  |> 
  ggplot(aes(x = loan_amount)) + geom_histogram(binwidth = 100000) +
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024",
       x = "Loan Amount",
       y = "Count of Loans",
...
hmda_ms_2024 |>
  ...
  ggplot(aes(x = loan_to_value_ratio)) + geom_histogram(binwidth = 10) +
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024: Loan to Value Ratios",
       x = "Loan to Value Ratio",
       y = "Count of Loans",
       caption = str_wrap("This histogram displays the distribution of the ratio of the loan amount to the purchase price of the home; basically, a score of financial risk for each Mississippi applicant in 2024. A higher LTV score usually leads to less borrower equity and greater risk. The plot shows that the majority of applicants have an LTV ratio of 80-100. Since the average LTV ratio is usually 80% (80% loan, 20% down payment), this indicates that a majority of applicants had slightly higher LTV ratios and therefore considered more risk for the lender.", 120)) +
  theme_minimal()
##TA Feedback: This graph looks fantastic, just a small fix: the y-axis again shows the count of applications not the count of loans granted -0.5 for mislabeling.
...
hmda_ms_2024 |>
  ggplot(aes(x = derived_race, fill = action_taken)) + geom_bar() +
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024: Race of Applicants and Action Taken",
...
##TA feedback: This graph looks okay, remember to label your fill variable! -1 for not labeling fill
...
hmda_ms_2024 |>
  ggplot(aes(x = derived_race, fill = action_taken)) + geom_bar(position = "fill") +
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024: Race of Applicants and Action Taken",
       x = "Race of Applicants",
       y = "Count of Loans",
       caption = str_wrap("This barplot displays the percentage of approvals for each race as well as the percentage of denials for each race for the Mississippi applicants in 2024. Based on the findings, white, asian, and applicants with race not available had the least amount of loan applications denied, and those who are Black or African American, American Indian or Alaska Native, and those with 2 or more minority races tend to have the most loan application denials.", 150)) +
  coord_flip() +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 6), 
        legend.title = element_text(size = 8),
        legend.key.size = unit(0.4, "cm")) +
  guides(fill=guide_legend(nrow=4))
##TA Feedback: Remember to label your legend regarding what variable the bars are colored/filled by, otherwise this graph looks fabulous. -1 for missing labels and the same minor labeling mistake as before.
...
hmda_ms_2024 |>
  ...
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024",
       x = "Race of Applicant",
       y = "Loan Amount",
       caption = str_wrap("Requested loan amounts vary across racial groups. The median loan amount appears highest among white applicants, while several minority groups show lower median requested amounts and narrower interquartile ranges, indicating potential disparities in loan demand or access in Mississippi during 2024.")) +
  theme_minimal() 
##TA Feedback: This graph look wondrous! +2.5 
##Total for Q1-Q7: 12.5 pts
```
...
hmda_denial_by_race |>
  ...
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024",
       x = "Race of Applicant",
       y = "Percent of Individuals Denied",
...
hmda_denial_by_race_sex |>
  ...
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024",
       ...
       y = "Percent of Individuals Denied",
       fill = "Sex", 
       caption = str_wrap("This column plot visualizes the denial rates of home mortgage applications in MS in 2024. The plot shows that Black or African American applicants, American Indian or Alaska Native applicants, or applicants of 2 or more minority races were all denied mortgage at double the rate of White applicants. The plot also shows that on average, female, joint sexes, and applicants with no sex available had a disproportionately higher rate of loan denial than male applicants. This may indicate disparities in lending practices.", 120)) +
  theme_minimal() +
  coord_flip()
##TA Feedback: This graph looks wonderful! +2.5
##Total for Q1-Q9: 17.5 pts
```
...
hmda_ms_2024 |>
  ...
  filter(derived_race != "Free Form Text Only") |>
  ggplot(aes(x = denial_reason, fill = derived_race)) +
  geom_bar(position = "dodge") +
  coord_flip() +
  labs(title = "Denied Mortgage Applications by Denial Reason and Race",
    subtitle = "Home Mortgage Disclosure Act Applications — Mississippi, 2024 (Denied applications only)",
    x = "Denial Reason",
    y = "Number of Denied Applications",
    fill = "Race of Applicant",
    caption = str_wrap("This plot displays the number of denied HMDA mortgage applications in Mississippi in 2024 by denial reason and applicant race. Each bar represents a count of applications with action_taken equal to 'Application denied.' Data source: HMDA, Mississippi, 2024.", 120)) +
  theme_minimal()
##TA Feedback: This graph looks terrific! +2.5
##Total for Q1-10: 20 pts
```
...
hmda_ms_2024 |>
  ...
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024",
       ...
       fill = "Action Taken", 
       caption = str_wrap("This bar plot visualizes the percent of mortgage applications that resulted in each action in Mississippi in 2024, separated by applicant race. While denial rates increase for applicants with debt-to-income ratios of 50% or higher across all races, Black or African American applicants show comparatively higher denial rates even among those with debt-to-income ratios below 50%. This suggests that disparities may persist beyond differences in debt burden alone.",width = 120)) +
  theme_minimal() +
  theme(legend.position = "bottom",
...
hmda_ms_2024 |>
  ...
  theme_minimal()
##TA Feedback: This graph looks good! Nice job! +2.5
##Total for Q1-Q12: 25 pts
```
...
##TA Grading: Final Score: 25/30 or 83.33% 
:::
