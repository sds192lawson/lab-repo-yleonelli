...
la_map <- leaflet(width = "100%") |>
  setView(lat = 30.5, lng = -91.5, zoom = 6) |>
  addProviderTiles("OpenStreetMap") # <- had to change the provider tile from the original template because it wasn't working
...
##TA feedback: try using unique like this next time! +2.5
#pal_fact <- colorFactor(palette = "Set2",
...
##TA Feedback: This look fabolous! +3
...
A: The colors appear differently on each map due to the way that each sequential color palette is categorized. In colorNumeric(), the values are assigned to a spectrum; in colorBin(), the values are assigned to different sections on an interval of 2,000,000; and in colorQuantile(), the values are assigned by grouping the numbers into equally sized quantiles--in this map, in terms of fourths and in percentages. The map using colorNumeric() is the best map to represent the distribution of values in X107..TOTAL.RELEASES because the values in this variable are numerical, so they should be mapped using a color palette that specifically deals with numerical values.
##TA feedback: Nice job on this answer! +3
...
la_map |> leaflet(width = "100%") |>
  ...
  addCircleMarkers(data = tri_la_2023,
                   ...
                   weight = 1,
                   stroke = TRUE,
                   popup = ~paste("<b>", X4..FACILITY.NAME, "</b><br><i>", X23..INDUSTRY.SECTOR, "</i><br>", X..CHEMICAL.TEXT),
