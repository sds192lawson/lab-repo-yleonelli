...
#| eval: false
library(tidycensus)
census_api_key("8babe0fcf3c20c7b225a7a9b6f67bbb8d87a4d5b", overwrite = TRUE, install = TRUE)
```
...
census_counties <- census_counties |>
  mutate("per_black" = (value / summary_value) *100)

census_tracts <- census_tracts |>
  mutate("per_black" = (value / summary_value) * 100)
##TA feedback: Nice job with this! +2
...
pal_bin_census_county_perc <- colorBin(palette="Blues", 
                    domain = census_counties$per_black,
                    bins = 5)
...
la_map %>%
  addPolygons(data = census_counties, 
              weight = 1,
              color = "black",
              fillColor = ~pal_bin_census_county_perc(per_black),
              fillOpacity = 0.5,) %>%
...
pal_bin_census_tract_perc <- colorBin(palette="Blues",
                                      domain = census_tracts$per_black,
                                      bins = 5)
...
la_map %>%
  ...
  addPolygons(data = census_tracts, 
              weight = 1,
              color = "black",
              fillColor = ~pal_bin_census_tract_perc(per_black),
              fillOpacity = 0.5) |>
...
A: The representation of higher populations of Black residents along the Mississippi River, as shown in the cities of Baton Rouge and New Orleans, is an example of the ecological fallacy because this was only shown after toggling "Tracts" on. If we were only to look at the "Counties" data, it would show that a majority of the Black population in Louisiana resided in the northeast, close to the border of Mississippi, but after turning "Tracts" on, we now see that the majority of them actually reside in the most populated cities, which are close to the Mississippi River.
##TA Feedback: Thoughtful answer! +3
...
A: The modifiable aerial unit problem could be at play here because by toggling "Counties" on, we do see that the areas where high-polluting industrial facilities are located ARE in higher-populated counties, but only looking at higher-percentage of Black population tracts zooms the picture in too much. We should definitely consider this when looking at pollution data because a majority of the time, pollution has a large area that it affects, and looking at the finer details doesn't allow you to see the full scale of said pollution.
