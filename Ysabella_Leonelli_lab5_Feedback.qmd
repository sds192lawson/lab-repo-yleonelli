...
A: Unit of observation --> NYC police stop cases
   Variables:
   - pct --> Precinct of the stop
   - race --> race of suspect
   - age --> age of suspect
   - frisked --> Was suspect frisked? (1 = Yes, 0 = No)
   - pistol, riflshot, asltweap, knifcuti, machgun, othrweap --> if suspect was
     holding any of the weapons above (1 = Yes, 0 = No)
   - sumissue --> if summons was issued (1 = Yes, 0 = No)
   - arstmade --> if arrest was made (1 = Yes, 0 = No)
   - race_cat --> race category of suspect
   - wpnfound (new variable) --> whether weapon was found on suspect (1 = Yes, 0      = No)
   - arrestsumm (new variable) --> Was an arrest made or summons issued? (1 =         Yes, 0 = No)
...
sqf_2011 |>
  ...
  summarize(total_innocent = n(), 
            percent_innocent = total_innocent#you can just use n() here instead / total_stops * 100) 
##TA Feedback: Otherwise this looks amazing! +2.5
##Total for Q1-Q4: 11.5
```
...
##TA Feedback: Looks like you figured it out this time! +3
...
total_stops_race_recorded <-
  ...
  summarize(Count = n()) |>
  pull()
##TA Feedback: Don't forget to sort stops by descending order! +2.5
...
sqf_2011 |>
  ...
  summarize(
    stops = n(),
    percent_stops = n() / nrow(sqf_2011) * 100,
    percent_frisked = mean(frisked == "1") * 100,
    percent_wpnfound = mean(wpnfound == "1") * 100,
    percent_arrestsumm = mean(arrestsumm == "1") * 100
  )
##TA Feedback: Use sum here not mean! +2
...
```{r, eval = FALSE}
According to the code above, Black and White-Hispanic people had disproportionately more stops than other race categories, with over 76% of stops coming from those two race categories. It also seems like the percent of people getting stopped and frisked in one occasion occurs more frequently in those of Black, Black-Hispanic, and White-Hispanic descent than other races, with White notably being the least frisked out of the racial categories. It also seems like, although Black and Hispanic people get stopped and frisked much more frequently, the percentage of weapons found and the percentage of arrests made or summons issued tend to be the same across the racial groups, with little variability.
##TA feedback: Super thoughful answer! +3
